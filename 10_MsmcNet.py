# ================================================================
#  MsmcNet — RADIOML 2016.10A
#  Wang et al. 2022 [18]: Modular few-shot learning framework for SMC
#  Architecture:
#    IQF module: Reshape(128,2,1) → Conv2D(64,(3,2),valid,relu) → Reshape(126,64)
#    4 × 1D-SFP (Signal Feature Processing) residual blocks:
#      Conv1D(64,3,same) → BN → ReLU → Conv1D(64,3,same) → BN → Add → ReLU
#    GlobalAveragePooling1D → Dense(nc,softmax)
#  Note: paper says GCN determines #layers; for RML2016.10A the fixed
#        optimal is 4 blocks (as used in comparison experiments)
#  Norm: global-max (paper: "AutoSMC adopts preprocessing consistent
#         with MsmcNet, remaining methods adopt per-sample max")
...
def sfp_block(x):
    """1D Signal Feature Processing residual block."""
    s = x
...
accs = []
print("Training MsmcNet — one fresh model per SNR")
for snr in SNRS:
...
ax.plot(SNRS, [a*100 for a in accs],
        marker='P', color='#545454', linewidth=1.8,
        markersize=6, label='MsmcNet')
ax.set_xlabel("SNR(dB)", fontsize=11)
ax.set_ylabel("Accuracy", fontsize=11)
ax.set_title("MsmcNet — RADIOML 2016.10A", fontsize=11)
ax.set_xticks(SNRS)
...
plt.tight_layout()
plt.savefig("MsmcNet_accuracy.png", dpi=150, bbox_inches='tight')
plt.show()
print("Saved → MsmcNet_accuracy.png")
