# ================================================================
#  VTCNN2 — RADIOML 2016.10A
#  O'Shea et al. 2016 [34]: VGG-style CNN on raw IQ signal
#  Architecture:
#    ZeroPad(2) → Conv1D(256,3,relu) → Dropout(0.5)
#    ZeroPad(2) → Conv1D(80,3,relu)  → Dropout(0.5)
#    Flatten → Dense(256,relu) → Dropout(0.5) → Dense(nc,softmax)
...
def build_model(nc):
    ...
    x   = layers.Conv1D(80, 3, activation='relu')(x)
    x   = layers.Dropout(0.5)(x)
    x   = layers.Flatten()(x)
...
accs = []
print("Training VTCNN2 — one fresh model per SNR")
for snr in SNRS:
...
ax.plot(SNRS, [a*100 for a in accs],
        marker='x', color='#6baed6', linewidth=1.8,
        markersize=6, label='VTCNN2')
ax.set_xlabel("SNR(dB)", fontsize=11)
ax.set_ylabel("Accuracy", fontsize=11)
ax.set_title("VTCNN2 — RADIOML 2016.10A", fontsize=11)
ax.set_xticks(SNRS)
...
plt.tight_layout()
plt.savefig("VTCNN2_accuracy.png", dpi=150, bbox_inches='tight')
plt.show()
print("Saved → VTCNN2_accuracy.png")
